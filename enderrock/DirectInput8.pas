(*)
 [------------------------------------------------------------------------------
 [  DirectX 8, 8.1, 9 Delphi Adaptation (c) by Tim Baumgarten
 ...
 [------------------------------------------------------------------------------
 [  This File contains Definitions for DInput8, 8.1 and 9 only.
...
(*)
 ...
 [ History :
 [----------
 [ 20-Dec-2002 (Tim Baumgarten) : Added DInput9. And Cleaned up a bit.
 [------------------------------------------------------------------------------
 [ 11-Nov-2001 (Tim Baumgarten) : Added DX8.1.
 [------------------------------------------------------------------------------
 [ 05-Nov-2001 (Tim Baumgarten) : Changed IDirectInput8{A/W}.SetDataFormat to
 [                                use a Pointer as Input
 [------------------------------------------------------------------------------
 [ 18-Jan-2001 (Tim Baumgarten) : Changed
 [                                IDirectInput8{A/W}.EnumDevicesBySemantics
 [                                and IDirectInput8{A/W}.ConfigureDevices to
 [                                use "const" instead of "var" for the records.
 [------------------------------------------------------------------------------
 [ 23-Dec-2000 (Tim Baumgarten) : Changed all types that are declared as
 [                                Cardinal in C to be Cardinal in Delphi
 [------------------------------------------------------------------------------
 [ 23-Dec-2000 (Tim Baumgarten) : Changed all types that are declared as
 [                                DWord in C to be LongWord in Delphi
 [------------------------------------------------------------------------------
...
{.$DEFINE STATIC_LINKING} //Do not define that ! it won't work yet
...
(*)
procedure ShowJoyCPL(WindowHandle : HWND);
// New for DX9 <-
...
// New for DX9 ->
procedure ShowJoyCPL(WindowHandle : HWND);
var ControlPanel : HMODULE;
var _ShowJoyCPL_ : procedure (WindowHandle : HWND);
begin
  ControlPanel := LoadLibrary('joy.cpl');

  _ShowJoyCPL_ := GetProcAddress(ControlPanel, 'ShowJoyCPL');
  _ShowJoyCPL_(WindowHandle);

  FreeLibrary(ControlPanel);
end;
// New for DX9 <-
